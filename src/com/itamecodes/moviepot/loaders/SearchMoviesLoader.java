...
public class SearchMoviesLoader extends WrappedAsyncTaskLoader<ArrayList<HashMap<String,String>>> {
	String query;
	static final String TAG="SearchMoviesLoader";
	public SearchMoviesLoader(Context context,String queryst) {
	...
	public ArrayList<HashMap<String,String>> loadInBackground() {
		...
		try {
			String url = "http://api.themoviedb.org/3/search/movie?api_key="
					+ Config.MDB.getKey() + "&query="+query+"&include_adult=false";
			Log.v(TAG,url);
			...
			for(DifferentMovies diffMovies:theList){
				...
				Log.v("vivektestnew",id+"--"+diffMovies.title);
